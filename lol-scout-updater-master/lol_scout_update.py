...
# read config file
absolute = '/Users/Utilisateur/DiscordBot/'
f = file(absolute+"sample_config.txt", "r")
...
def changeTextFile(sum_name, region, champ, sum1, sum2, item1, item2, item3, item4, item5, item6, rune_list, champion_id, games, wins, losses, kills, deaths, ast):
	s_rune = ''
...
def getSummonerInfo(j_list):
	...
	for j in j_list:
		...
		if len(participants['runes']) > 8 or len(participants['runes']) < 4:
			continue
                if participants['championId'] in potential_champions:
                    continue
...
def getChampName(id):
	...
	if champ_id_r.status_code == 200:
		...
		raise KeyError('Could not connect when getting the name of the champion for the id = ' + str(highest_champ_id))
...
def meets_requirements(sum_name):
	...
	if id_r.status_code == 200:
		...
		sum_id = id_j[sum_name_stripped]['id']
                ...
                ml_r = requests.get('https://' + region + '.api.pvp.net/api/lol/' + region + '/v2.2/matchlist/by-summoner/' + str(sum_id) + '?api_key=' + key)
                match_ids = get10MatchIds(ml_r.json())
                match_info_json = []
                for ids in match_ids:
                    time.sleep(3)
                    m_r = requests.get('https://' + region + '.api.pvp.net/api/lol/' + region + '/v2.2/match/' + str(ids) + '?api_key=' + key)
                    try:
                        match_info_json.append(m_r)
                    except Exception as e:
                        print(m_r.text)
                
	        potential_champions, item_builds, summoner_spells, rune_list = getSummonerInfo(match_info_json)
		...
		if rs_r.status_code == 200:
		    ...
		    changeTextFile(sum_name, region, champ_name, summoner_spell1, summoner_spell2, item_build_name[0], item_build_name[1], item_build_name[2], item_build_name[3], item_build_name[4], item_build_name[5], rune_page, champion_id, games, wins, losses, kills, deaths, ast)
		    return True
		else:
